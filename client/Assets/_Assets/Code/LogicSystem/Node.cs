...
namespace SF.LogicSystem.v2
{
    public class Node : MonoBehaviour
    {
        ...
        {
            bool hasTwoNodes = levelEntity.outputNode != null && levelEntity.inputNode != null;
        ...
        protected Node FindClosestNode(Vector2 position, bool allowSelf)
        {
            overlappingNodes = new Collider2D[9];
            if (Physics2D.OverlapPointNonAlloc(position, overlappingNodes, whatIsNode) > 0)
            {
                ...
                foreach (Collider2D nodeCollider in overlappingNodes)
                {
                    ...
                    if( levelEntity.inputNode != null )
                        if( nodeCollider == levelEntity.inputNode.collider )
                            continue;
                ...
                return closestNode.GetComponentInParent<Node>();
            }
            return null;
        }
        ...
        public bool AlreadyLinked(Node other)
        {
            foreach(Link link in links)
            {
